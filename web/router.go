...
路由查找会回溯吗?
这也是和 Web 框架相关的，我们在课程上是不支持的。在这里可以简单描述可回溯和不可回溯之间的区别，可以是田课程体user/123/home 和 /user/*\/*。我这里不支持是因为这个特性非常鸡肋。
...
具体匹配方式的实现原理。
课程上我们讨论了静态匹配、通配符匹配和路径匹配, 作业里面要求大家照着实现一个正则匹配。其实核心就是划定优先级, 然后一种种匹配方式挨个匹配过去。
